...
GitHub IssueとGitの実装情報を収集し、Codex CLIでPull Requestのタイトルと本文を生成してGitHub Pull Requestを作成します。

```bash
./scripts/create-pull-request.sh 60
```

baseブランチは`dev`が既定値で、必要な場合は`--base`で変更できます。headブランチには常に現在チェックアウト中のブランチを使用します。

```bash
./scripts/create-pull-request.sh 60 --base main
```

スクリプトは既存のPull Request Draft Prompt TemplateをCodex CLIへ渡します。ローカルにPR Draftを保存せず、生成したタイトル、本文、base/headブランチを表示して、Humanが明示的に確認した場合のみPull Requestを作成します。
...
7. Review
8. Commit
9. `./scripts/create-pull-request.sh <issue-number>`
10. Merge
11. `./scripts/finish-issue.sh`
