...
HumanがレビューしたPull Request DraftからGitHub Pull Requestを作成します。

PR Draftは恒久保存せず、Humanがレビューした一時ファイルを`--draft`で明示的に指定します。Pull Request作成後はGitHub上のPull Requestを正として管理します。

```bash
./scripts/create-pull-request.sh 60 --draft /tmp/pr-draft-60.md
```

baseブランチは`dev`、headブランチは現在のブランチが既定値です。必要な場合はオプションで変更できます。

```bash
./scripts/create-pull-request.sh 60 --draft /tmp/pr-draft-60.md --base main --head feature/60-example
```

スクリプトはタイトル、本文、base/headブランチを表示し、Humanが明示的に確認した場合のみPull Requestを作成します。
...
10. `./scripts/create-pull-request.sh <issue-number> --draft <temporary-file>`
11. Merge
